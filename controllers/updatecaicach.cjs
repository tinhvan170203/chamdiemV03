...
module.exports = {
    getPhieuchams: async (req, res) => {
        ...
        try {
            let items = await PhieudiemNew.find({ user_created: id_user }).sort({ createdAt: -1 });
    ...
    createdCuocchamdiem: async (req, res) => {
        ...
        try {
            ...
            await item.save();
            let items = await QuantriNamChamdiem.find({user_created: req.body.user_created}).populate('setting.phieucham',{name: 1}).sort({nam: -1})
    ...
    changeStatusChotdiemGiaitrinh: async (req, res)=>{
        ...
        try {
            ...
            await phieucham.save();
            res.status(200).json({message: "Thay đổi trạng thái tự giải trình điểm tự chấm thành công"})
        } catch (error) {
            console.log("lỗi: ", error.message);
            res.status(401).json({
                status: "failed",
    ...
    checkPhieuchamUsed: async (req, res) => {
        ...
        try {
            let check_nam_cham_diem_used = await QuantriNamChamdiem.find({
                "setting.phieucham": id_phieucham
            });
