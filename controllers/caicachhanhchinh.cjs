...
// console.log(checked_namchamdiem)
      if (!checked_namchamdiem) {
        ...
        let item = await Phieuchamdiems.findOne({ year, taikhoan: req.params.id }).populate('phieuchamdiem');
  ...
  saveChotdiemGiaitrinh: async (req, res) => {
    ...
    try {
      ...
      await saveAction(req.userId.userId, `Chốt phần giải trình`)
  ...
  fetchBangchamthamdinh: async (req, res) => {
    ...
    try {
      let item = await Phieuchamdiems.findOne({ year, taikhoan: taikhoan }).populate('phieuchamdiem')
      if (!item) {
...
// console.log(phieucham)
  ...
  trackingBangdiem: async (req, res) => {
    let { year } = req.query;
    try {
      let items = await Phieuchamdiems.find({ year }).populate("taikhoan");
      // console.log(items)
  ...
  saveUploadtailieuDiemthuong: async (req, res) => {
    ...
    try {
      ...
      res.status(200).json({ message: "Lưu tài liệu kiểm chứng thành công", files: files, ghichucuadonvi })
    } catch (error) {
      console.log("lỗi: ", error.message);
      res.status(401).json({
        status: "failed",
  ...
  updateGhichuthamdinhDiemthuong: async (req, res) => {
    ...
    try {
      ...
      await phieucham.save();
      res.status(200).json({ message: "Lưu ghi chú thẩm định thành công", ghichucuathamdinh })
    } catch (error) {
      console.log("lỗi: ", error.message);
      res.status(401).json({
        status: "failed",
  ...
  fetchLichsuHethong: async (req, res) => {
    try {
      let { tentaikhoan, tungay, denngay, action } = req.query;
      if (tungay === "") {
